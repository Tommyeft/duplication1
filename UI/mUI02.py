...
import dlib  # 导入 dlib 库

# 加载 dlib 的人脸检测器和特征提取器
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(
    "E:\\desk\\biyesheji\\duplication1\\.venv\\Lib\\site-packages\\face_recognition_models\\models\\shape_predictor_68_face_landmarks.dat")
face_rec_model = dlib.face_recognition_model_v1(
    "E:\\desk\\biyesheji\\duplication1\\.venv\\Lib\\site-packages\\face_recognition_models\\models\\dlib_face_recognition_resnet_model_v1.dat")
...
def get_dlib_face_encoding(image, face_rect):
    """使用dlib获取人脸编码"""
...
class FaceRecognitionWindow(QWidget):
    ...
    def import_id(self):
        ...
        try:
            ...
            faces = detector(gray, 1)  # dlib人脸检测
            if not faces:
                QMessageBox.warning(self, "警告", "未检测到人脸，请选择包含人脸的图片！")
                return
            face_rect = faces[0]
            face_encoding = get_dlib_face_encoding(image, face_rect)
                ...
                while True:
                    ...
                    for box in results[0].boxes:
                        if box.cls == 0:
                            ...
                            # 1. 使用dlib获取人脸编码
                            try:
                                face_encoding = get_dlib_face_encoding(rgb_frame, face_rect)
                            except Exception as e:
                                print(f"dlib编码错误: {e}")
                                continue

                            # 2. 与数据库中的编码比对
                            known_encodings = [np.array(enc) for enc in self.face_data.get("encodings", [])]
                            ...
                            if True in matches:
                                ...
                                display_name = self.face_data["ids"][matched_index]

                            # 检查该人脸是否已经被识别过
                            face_key = (x1, y1, x2, y2)
                            if face_key not in recognized_faces:
                                recognized_faces[face_key] = display_name

                            # 4. 绘制结果（添加ID显示）
                            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                            font_scale = min((x2 - x1) / 100, 1.2)
                            cv2.putText(frame, recognized_faces[face_key], (x1, y2 + 30),
                    ...
                    # 移除已经离开摄像头范围的人脸记录
                    faces_to_remove = []
                    for face_key in recognized_faces.keys():
                        if face_key not in current_faces:
                            faces_to_remove.append(face_key)
                    for face_key in faces_to_remove:
                        del recognized_faces[face_key]
    ...
    def recognize_face_image(self):
        """使用 YOLO 进行图片人脸识别，识别后调用 dlib 库显示输入的 ID"""
        options = QFileDialog.Options()
        ...
        try:
            ...
            known_encodings = self.face_data.get("encodings", [])
            known_ids = self.face_data.get("ids", [])
            ...
            for box in result[0].boxes:
                if box.cls == 0:
                    ...
                    if True in matches:
                        matched_index = matches.index(True)
                        name = known_ids[matched_index]

                    # 显示 ID
                    cv2.putText(image, name, (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), 2)
